...
export class TokenServiceService {
      ...
      .subscribe((response) => {
        ...
        if (response && response.accessToken) {
          ...
          localStorage.setItem(
            'expiryDuration',
            response.expiryDuration.toString()
          );
  ...
  getExpiration() {
    const expiration = Number(localStorage.getItem('expiryDuration'));
    const expiresAt = moment().valueOf() + expiration;
    return moment(expiresAt);
  }
  refreshToken(): Observable<LoginResponse> {
    if (moment().valueOf() >= this.getExpiration().valueOf()) {
      this.router.navigate(['/login']);
    } else {
      const refreshToken = localStorage.getItem('refreshToken');
      const citizenId = localStorage.getItem('citizen_id');
      return this.http
        .post<LoginResponse>(`http://localhost:8080/api/v1/auth/refreshtoken`, {
          refreshToken,
          citizenId,
        })
        .pipe(
          tap((res: LoginResponse) => {
            ...
            const expiresAt = new Date().getTime() + res.expiryDuration;
